...
def main():
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    wer, loss = train(encoder, decoder, char_decoder, optimizer, scheduler, criterion, grad_scaler, train_loader, text_transform, args, gpu=gpu) 
    valid_wer, valid_loss = validate(encoder, decoder, char_decoder, criterion, test_loader, text_transform, args, gpu=gpu)
...
def train(encoder, decoder, char_decoder, optimizer, scheduler, criterion, grad_scaler, train_loader, text_transform, args, gpu=True):
  ...
  encoder.train()
  decoder.train()
  for i, batch in enumerate(tqdm(train_loader, total=len(train_loader), desc="Training", leave=False)):
    scheduler.step()
    gc.collect()
    spectrograms, labels, input_lengths, label_lengths, references, mask = batch 

    # print("Max input length:", input_lengths.max().item())
    # print("Max label length:", label_lengths.max().item())
    # print("Any invalid:", (input_lengths < label_lengths).any())
...
#           print("NaN detected in outputs BEFORE loss")
#           print("Outputs shape:", outputs.shape)
#           return 0, 0

#       if torch.isinf(outputs).any():
#           print("Inf detected in outputs BEFORE loss")
#           return 0, 0

#       print("Model output shape:", outputs.shape)
#       print("Max input_length:", input_lengths.max().item())
# ##########################################################################
...
def validate(encoder, decoder, char_decoder, criterion, test_loader, text_transform, args, gpu=True):
