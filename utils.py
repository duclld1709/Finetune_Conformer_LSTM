...
class TextTransform:
    ...
    def __init__(self):
        # Bảng chữ cái tiếng Việt đầy đủ (chữ thường) + các ký tự bổ trợ
        # Bao gồm: 29 chữ cái, các chữ có dấu thanh, khoảng trắng và dấu nháy
        vi_alphabet = (
        ...
        self.char_map = {}
        for i, char in enumerate(vi_alphabet):
        ...
        # Token trống (blank) cho CTC Loss - thường đặt ở vị trí cuối cùng
        self.blank_idx = len(vi_alphabet) 

    def text_to_int(self, text):
        ''' Chuyển chuỗi văn bản thành dãy số nguyên '''
        # Chuẩn hóa Unicode (NFC) và chuyển về chữ thường
        ...
        for c in text:
            if c in self.char_map:
                ...
                # Bỏ qua ký tự lạ hoặc có thể map vào một token <UNK> nếu muốn
                continue
        return int_sequence

    def int_to_text(self, labels):
        ''' Chuyển dãy số nguyên thành chuỗi văn bản '''
        string = []
        for i in labels:
            if i == self.blank_idx: # Blank token
                continue
...
class GreedyCharacterDecoder(nn.Module):
  ''' Greedy CTC decoder - Argmax logits, remove duplicates, drop blanks. '''
  ...
  def forward(self, x, lengths=None):
    ...
    if lengths is not None:
      if not torch.is_tensor(lengths):
        ...
        lengths = lengths.to(indices.device)
      max_len = indices.size(1)
      valid_positions = torch.arange(max_len, device=indices.device).unsqueeze(0) < lengths.unsqueeze(1)
      indices = torch.where(valid_positions, indices, torch.full_like(indices, self.blank_idx))
    ...
    for sample in indices.tolist():
      if self.blank_idx is None:
        ...
        decoded.append([token for token in sample if token != self.blank_idx])
    return decoded
